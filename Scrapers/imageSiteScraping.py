...
targetURL = 'http://[REDACTED].com/forum-36-6.html'
os.makedirs('[REDACTED]6', exist_ok = True)
...
for i in range(30):
    threadURL.append('http://[REDACTED].com/' + threadElem[i].get('href'))
...
for url in threadURL:
    ...
    # Create Folder within [REDACTED]
    print("Folder Created: %s..." % (folderName))
    os.makedirs('[REDACTED]6/' + folderName, exist_ok = True)
    ...
    if picElem == []:
        ...
        for i in range(len(picElem)):
            picURL = 'http://[REDACTED].com/' + picElem[i].get('file')
            print('Downloading image %s...' % (picURL))
            res = requests.get(picURL)
            res.raise_for_status()

            imageFile = open(os.path.join('[REDACTED]6',folderName, os.path.basename(picURL)), 'wb')
